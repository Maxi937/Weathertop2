...
const weatherAnalytics = {
  ...
  getWeather(reading) {
    ...
    try {
      weather = {
        ...
        icon: this.searchForIcon(reading.autoWeatherData.main)
      };
    } catch(error){
      for (let i = 0; i < weatherStore.length; i++) {
        if (reading.code == weatherStore[i].code) {
          weather = {
            code: reading.code,
  ...
  //Will Try to find a match for the weather generated by auto reading
  searchForIcon(weatherDescription){
    const iconStore = weatherAnalyticsStore.weatherCodes;

    for (var i=0 ; i < iconStore.length ; i++) {
      if (iconStore[i].weather == weatherDescription) {
          return iconStore[i].icon;
        }
      if (iconStore[i].weather.includes(weatherDescription)){
        return iconStore[i].icon;
      }
    }
    return 'orange first order'
  }
};
